...
image_path = "resources/lena_color_512.tif"
...
peppers_image = cv2.imread("resources/peppers_color.tif")
peppers_gray = cv2.cvtColor(peppers_image, cv2.COLOR_BGR2GRAY)
x, y, w, h = 100, 100, 200, 200
peppers_cropped = peppers_gray[y : y + h, x : x + w]
cv2.imwrite("resources/peppers_cropped.jpg", peppers_cropped)

# Ejercicio 7: Leer y desplegar imagen RAW
width, height = 800, 600
raw_image_data = np.fromfile("resources/rosa800x600.raw", dtype=np.uint8)
...
cap = cv2.VideoCapture("resources/0X2A8498756C4D6E82_corto.avi")
while True:
    ret, frame = cap.read()
    if not ret:
        break
